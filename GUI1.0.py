...
class main:
    ...
    def widgets(self):  # widgets of the calendar drop
        ...
        mon = ttk.Combobox(f, width=7, font=('freesansbold', 13), values=self.months, textvariable=self.mon)
        ...
        f.pack()
        self.area = Text(self.master, font=('', 15, 'freesansbold'), width=20, height=9, bd=15)
        self.area.pack()

        # button set to the getcal function
        Button(self.master, command=self.getcal, text='Get calendar', font=('', 15, 'freesansbold'), bd=10).pack()
